...
begin
    if rising_edge(clk) then
        if rst = '1' then
            ...
            PC_int <= std_logic_vector(unsigned(toPC) - 4); -- unidade de controle traz o valor + 4, então é como se tivesse dado um stall
        else
            PC_int <= toPC;
        end if;
    end if;
...
-- Writeback
ResultOut <= Mux_4x2(ALUResult_int, ReadData, PCPlus4, ImmExt, ResultSrc);
